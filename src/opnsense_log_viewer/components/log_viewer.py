"""
Main OPNsense Log Viewer Application component.
"""
...
from opnsense_log_viewer.constants.app_constants import (
    ...
    TIME_FORMAT, APP_NAME, APP_VERSION, APP_URL
)
...
class LogViewerApp:
    ...
    def __init__(self, root):
        ...
        self.ssh_port = tk.StringVar(value=str(DEFAULT_SSH_PORT))
    ...
    def setup_filter_panel(self, parent):
        ...
        self.field_combo = ttk.Combobox(add_filter_frame, textvariable=self.field_var,
                                       values=FILTER_FIELD_OPTIONS, width=15)
        ...
        operator_combo = ttk.Combobox(add_filter_frame, textvariable=self.operator_var,
                                    values=FILTER_OPERATOR_OPTIONS, width=10)
        ...
        logic_combo = ttk.Combobox(add_filter_frame, textvariable=self.logic_var,
                                 values=FILTER_LOGIC_OPTIONS, width=8)
    ...
    def setup_log_table_tab(self, notebook):
        ...
        page_size_combo = ttk.Combobox(pagination_frame, textvariable=self.page_size_var,
                                      values=PAGE_SIZE_OPTIONS, width=8)
        ...
        self.log_tree = ttk.Treeview(tree_frame, columns=LOG_TABLE_COLUMNS, show='headings', height=20)

        # Column configuration
        for col in LOG_TABLE_COLUMNS:
            self.log_tree.heading(col, text=COLUMN_HEADERS[col], command=lambda c=col: self.sort_column(c))
        ...
        self.log_tree.bind('<Double-1>', self.on_cell_double_click)
    ...
    def refresh_display(self):
        ...
        for entry in self.displayed_entries:
            ...
            values = (
                entry.timestamp.strftime(TIME_FORMAT) if entry.timestamp else '',
                entry.get('action', ''),
    ...
    def _show_file_tail(self):
        ...
        try:
            ...
            # Read last lines directly from file (fast)
            last_lines = read_file_tail(file_path, TAIL_READ_LINES)

            # Parse only these lines
            self.displayed_entries = []

            for line in last_lines[-self.page_size:]:
        ...
        try:
            ...
            if 0 <= col_index < len(values):
                ...
                if cell_value:
                    ...
                    if col_index < len(LOG_TABLE_COLUMNS):
                        col_name = COLUMN_HEADERS.get(LOG_TABLE_COLUMNS[col_index], LOG_TABLE_COLUMNS[col_index])
        ...
        if self.time_filter_enabled.get():
            try:
                ...
                if self.start_time_var.get():
                    start_time = datetime.strptime(self.start_time_var.get(), TIME_FORMAT)

                if self.end_time_var.get():
                    end_time = datetime.strptime(self.end_time_var.get(), TIME_FORMAT)
    ...
    def show_about(self):
        ...
        messagebox.showinfo("About",
                          ...
                          f"Advanced log viewer for OPNsense firewall logs\n"
                          f"{APP_URL}\n\n"
    ...
    def connect_ssh_and_get_labels(self):
        ...
        def ssh_worker():
            ...
            try:
                ...
                success, message = self.ssh_client.connect(address, username, password, port=port, timeout=SSH_TIMEOUT)
                ...
                success, message, label_descriptions = self.ssh_client.extract_rule_labels(timeout=SSH_TIMEOUT)
