...
class ParallelFilterWorker:
    """Worker for parallel filtering"""
    
    @staticmethod
    def filter_chunk_data(args):
        """Filters a data chunk (static function for multiprocessing)"""
        try:
...
class ParallelLogFilter:
    ...
    def apply_filter_parallel(self, virtual_log_manager, filter_func, progress_callback=None, rule_labels_mapping=None):
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit all jobs
                future_to_chunk = {
                    executor.submit(ParallelFilterWorker.filter_chunk_data, args): args[3] 
    ...
    def _fallback_sequential_filter(self, virtual_log_manager, filter_func, progress_callback=None):
        ...
        if progress_callback:
            progress_callback("Using sequential mode...")
        
        # Use the original method
        virtual_log_manager.apply_filter(filter_func, progress_callback)
